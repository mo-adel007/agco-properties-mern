...
export const getProjectsByQuery = async (req, res, next) => {
  try {
    const { type, category, bedrooms, location, deliveryDate } = req.query;
    let filter = {};

    if (type) filter.type = type;
    if (category) filter.typeOfUnit = category;
    if (bedrooms && type !== "New Projects") filter.bedrooms = bedrooms;
    if (deliveryDate && type === "New Projects") filter.deliveryDate = deliveryDate;
