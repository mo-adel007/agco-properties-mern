...
export default function UpdateProject() {
  ...
  const fetchCommunities = async (developer, selectedCommunity) => {
    try {
      const res = await fetch(`http://localhost:3000/api/community/communities/developer/${developer}`);
      const data = await res.json();
      setCommunities(data);

      // Set the community value after fetching the communities
      if (selectedCommunity) {
        setFormData((prevData) => ({ ...prevData, community: selectedCommunity }));
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(true);
      setError(false);
      const res = await fetch(`http://localhost:3000/api/project/update/${params.projectId}`, {
        method: "POST",
        headers: {
