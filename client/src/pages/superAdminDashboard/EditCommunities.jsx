...
export default function EditCommunities() {
  ...
  useEffect(() => {
    const fetchCommunity = async () => {
      try {
        ...
        // Map the fields correctly to formData
        setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(false);
      const res = await fetch(
        `http://localhost:3000/api/community/update/${params.communityId}`,
        {
          method: "PUT",
          headers: {
      ...
      const data = await res.json();
      setLoading(false);
      toast.success("The community is updated!")
      if (data.success === false) {
        setError(data.message);
        toast.error("There was a problem while editing the listing")
      }
